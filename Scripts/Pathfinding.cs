...
public class Pathfinding {
    ...
    public List<TNode> GetPath<TNode>(TNode startingNode, TNode endNode, TNode[,] nodes) where TNode : IGridTile<TNode> {
    ...
    public List<IGridTile<TNode>> GetPath<TNode>(IGridTile<TNode> startingNode, IGridTile<TNode> endNode, IGridTile<TNode>[,] nodes) {
        List<IGridTile<TNode>> _openList = new List<IGridTile<TNode>>(), _closedList = new List<IGridTile<TNode>>();
        _openList.Clear();
        _closedList.Clear();
        ...
        for (int w = 0; w < nodes.GetLength(0); w++) {
            for (int h = 0; h < nodes.GetLength(1); h++) {
                IGridTile<TNode> current = nodes[w, h];
                current.WalkingCost = int.MaxValue;
                current.CameFrom = default;
            }
        }
        ...
        startingNode.HeuristicCost = CalculateDistanceCost(startingNode, endNode);
        while (_openList.Count > 0) {
            IGridTile<TNode> current = GetLowestTotalCostNode<IGridTile<TNode>>(_openList);
            if (current.Equals(endNode))
                return CalculatePathResult(endNode);
            _openList.Remove(current);
            _closedList.Add(current);
            foreach (IGridTile<TNode> neighbour in GetNeigbours(current, nodes)) {
                if (_closedList.Contains(neighbour) || neighbour.CanBeNavigated == false)
                    continue;
                int tentativeWalkingCost = current.WalkingCost + CalculateDistanceCost(current, neighbour);
                if (tentativeWalkingCost < neighbour.WalkingCost) {
    ...
    private List<IGridTile<TNode>> GetNeigbours<TNode>(IGridTile<TNode> target, IGridTile<TNode>[,] nodes) {
        List<IGridTile<TNode>> results = new List<IGridTile<TNode>>();
    ...
    private List<TNode> GetNeigbours<TNode>(TNode target, TNode[,] nodes) where TNode : IGridTile<TNode> {
    ...
    private List<TNode> CalculatePathResult<TNode>(TNode endNode) where TNode : IGridTile<TNode> {
        List<TNode> path = new List<TNode>();
        path.Add(endNode);
        IGridTile<TNode> current = endNode;
    ...
    private List<IGridTile<TNode>> CalculatePathResult<TNode>(IGridTile<TNode> endNode) {
        List<IGridTile<TNode>> path = new List<IGridTile<TNode>>();
        path.Add(endNode);
        IGridTile<TNode> current = endNode;
        while (current.CameFrom != null) {
    ...
    private int CalculateDistanceCost<TNode>(TNode startNode, TNode endNode) where TNode : IGridTile<TNode> {
    ...
    private int CalculateDistanceCost<TNode>(IGridTile<TNode> startNode, IGridTile<TNode> endNode) {
        int xDistance = (int)Mathf.Abs(startNode.GetPosition.x - endNode.GetPosition.x);
        int yDistance = (int)Mathf.Abs(startNode.GetPosition.y - endNode.GetPosition.y);
        int remaining = (int)Mathf.Abs(xDistance - yDistance);
        return MOVE_DIAGONAL_COST * Mathf.Min(xDistance, yDistance) + MOVE_STRAIGHT_COST * remaining;
    }
    private TNode GetLowestTotalCostNode<TNode>(List<TNode> nodes) where TNode : IGridTile<TNode> {
    ...
    private IGridTile<TNode> GetLowestTotalCostNode<TNode>(List<IGridTile<TNode>> nodes) {
        IGridTile<TNode> result = nodes[0];
        for (int i = 1; i < nodes.Count; i++) {
            if (nodes[i].TotalCost < result.TotalCost)
                result = nodes[i];
        }
    ...
    public interface IGridTile<T> {
        ...
        IGridTile<T> CameFrom { get; set; }
        GameObject gameObject { get; set; }
        T GetNewNode(Func<T> constructor) {
